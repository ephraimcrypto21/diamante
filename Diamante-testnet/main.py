...
╚═════╝░╚═╝╚═╝░░╚═╝╚═╝░░░░░╚═╝╚═╝░░╚═╝╚═╝░░╚══╝░░░╚═╝░░░╚══════╝
    ...
    print_border("DIAMANTE TESTNET", Fore.GREEN)
...
async def run_transfer(language: str):
    from scripts.transfer import run_transfer as transfer_run
    await transfer_run(language)
...
def get_available_scripts(language):
    scripts = {
        'vi': [
            ...
            {"name": "2. Gửi TX ngẫu nhiên hoặc File (address.txt)", "value": "transfer"},

            {"name": "X. Thoát", "value": "exit"},
        ],
        'en': [
            {"name": "1. Faucet $DIAM", "value": "faucet"},
            {"name": "2. Send TX random or File (address.txt)", "value": "transfer"},
            {"name": "X. Thoát", "value": "exit"},
        ]
    }
...
def main():
    ...
    messages = {
        "vi": {
            ...
            "locked": "🔒 Script này bị khóa! Vui lòng tham gia nhóm của chúng tôi để biết thêm thông tin hoặc mua để mở khóa."
        },
        "en": {
            "running": "Running: {}",
            "completed": "Completed: {}",
    ...
    while True:
        ...
        if selected_script.get("locked"):
            ...
            print(f"{Fore.CYAN}→ Website: https://thogtoolhub.pages.dev/{Style.RESET_ALL}")
            print('')
